...
public class MailService {
    ...
    public Map<String, Object> verify(String token) {
        ...
        try {
            ...
            Optional<UserRole> optionalUserRole = userRoleRepository.getUserByUserIdAndRoleId(userId, 3);
            if (optionalUserRole.isEmpty()) {
                resultMap = Map.of("status", "failed", "message", "이미 인증 완료된 이메일입니다.");
            } else {
                userRoleRepository.updateRoleId(optionalUserRole.get().getUserRoleId(), userId);
                resultMap = Map.of("status", "success", "message", "이메일이 인증이 완료되었습니다.");
            }
